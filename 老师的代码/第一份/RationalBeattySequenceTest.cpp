...
void testSequence(boost::rational<int> ratio, int dir) {  //这里定义了一个函数  ratio是int有理数斜率  dir是int
    int i;

    RationalBeattySequence bs(ratio, dir);//声明一个对象，输入斜率和一个整数
    cout << "Sequence: " << bs << endl; //bs会返回一个值
    for (i = 1; i < 15; i++) {
	cout << bs(i) << ' ';
    }
    cout << endl << endl;
